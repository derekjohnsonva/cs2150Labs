...
int main (int argc, char* argv[]) {
  ...
  // the last step in determining the size of the hash table will
  // be multiplying the numOflines by the load Factor
  double loadFact = .5;
  numOfLines =(double)numOfLines*loadFact;
      ...
      for(int dir = 0; dir<=7; dir++) {
        for(int len = 3; len<=22 ;len++){
          ...
          if(hashDict.checkTable(word)){
            cout << directionList[dir] <<"("<<r<<", "<<c<<"):         "<<word<<endl << flush;
            foundWords++;
